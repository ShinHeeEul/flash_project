...
public class DementiaController {
    ...
    @PostMapping("/upload")
    //@ResponseBody
    public ResponseEntity<ArrayList<TextbySpeaker>> saveFile(@RequestParam("voice_file") MultipartFile voice_file,
        ...
        if(!voice_file.isEmpty()) {
            ...
            if((voice_file.getContentType().equals(FILETYPE.M4A.getType())) ||
                ...
                WebSocketClient webSocketClient = new WebSocketClient(resultRepository);
                webSocketClient.analysisSTT(result, resultRepository);
    ...
    @ResponseBody
    public String ModifiedText(@ModelAttribute("result") TextbySpeakerList textbySpeakerList) throws DeploymentException, IOException {
